...
public class InitialDataSeeder : IInitialDataSeeder
{
    ...
    private async Task SeedCategorias(AppDbContext db)
    {
        ...
        foreach (var nombre in categorias)
            db.Categorias.Add(new Categoria { Nombre = nombre, Tipo = TipoCategoria.Egreso });
    ...
    private async Task SeedEmpleados(AppDbContext db)
    {
        ...
        foreach (var e in empleados) db.Empleados.Add(e);
        await db.SaveChangesAsync();
    }
    ...
    private async Task SeedServicios(AppDbContext db)
    {
        ...
        foreach (var s in servicios) db.Servicios.Add(s);
        await db.SaveChangesAsync();
    }
    ...
    private async Task SeedProductos(AppDbContext db)
    {
        ...
        foreach (var p in productos) db.Productos.Add(p);
        await db.SaveChangesAsync();
    }
    ...
    private async Task SeedClientes(AppDbContext db)
    {
        var clientes = new[]
        {
            ...
            new Cliente { Nombre = "Sofía Morales",    Telefono = "987654323", Email = "sofia@example.com",    Observaciones = "Viene para tratamientos. Alérgica a químicos",  FechaRegistro = DateTime.UtcNow },
        ...
        foreach (var c in clientes) db.Clientes.Add(c);
        await db.SaveChangesAsync();
    }

    private async Task SeedPaquetes(AppDbContext db)
    {
        var servicioIds = (await db.Servicios.ToListAsync()).Select(s => s.Id).ToList();
        ...
        db.Paquetes.AddRange(paquete1, paquete2);
        await db.SaveChangesAsync();
    ...
    private async Task SeedIngresos(AppDbContext db)
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            if (tipoRandom < 50)
            {
                ...
                {
                    Fecha = fecha,
                    Tipo = TipoIngreso.Servicio,
                    Monto = servicio.Precio,
                    Descuento = descuentoEspecial > 0 ? Math.Round(servicio.Precio * descuentoEspecial / 100m, 2) : 0m,
                    MetodoPago = new[] { MetodoPago.Efectivo, MetodoPago.Yape, MetodoPago.Plin, MetodoPago.Transferencia }[random.Next(4)],
            ...
            else if (tipoRandom < 75)
            {
                ...
                {
                    Fecha = fecha,
                    Tipo = TipoIngreso.Producto,
                    Monto = producto.PrecioVenta * cantidad,
                    Descuento = 0m,
                    MetodoPago = new[] { MetodoPago.Efectivo, MetodoPago.Pos }[random.Next(2)],
                    Comision = 0m,
            ...
            else
            {
                ...
                {
                    Fecha = fecha,
                    Tipo = TipoIngreso.Personalizado,
                    Monto = Math.Round((decimal)(20 + random.Next(1, 60)), 2),
                    Descuento = 0m,
                    MetodoPago = new[] { MetodoPago.Efectivo, MetodoPago.Transferencia, MetodoPago.Yape }[random.Next(3)],
                    Comision = 0m,
        ...
        foreach (var ingreso in ingresos) db.Ingresos.Add(ingreso);
        await db.SaveChangesAsync();
    }
    ...
    private async Task SeedEgresos(AppDbContext db)
    {
        ...
        var categoriasList = await db.Categorias
            .Where(c => c.Tipo == TipoCategoria.Egreso || c.Tipo == TipoCategoria.Ambos)
            .ToListAsync();
        var proveedores = new[] { "Distribuidora Beauty Plus", "Insumos Profesionales Inc", "Empresa de Agua Local", "Electricidad Nacional", "Google Ads", "Facebook Ads", "Reparaciones Locales", "Proveedor de Equipos" };
        var descripciones = new[] { "Compra de insumos y productos", "Pago de servicios", "Pago de salarios", "Pago de renta", "Gastos de marketing y publicidad", "Mantenimiento de equipos", "Gastos generales" };
        ...
        {
            var fecha = DateTime.UtcNow.AddDays(-random.Next(0, 60));
            var categoria = categoriasList[random.Next(categoriasList.Count)];

            decimal monto = categoria switch
            {
                { Nombre: "Insumos y productos" } => Math.Round((decimal)(15 + random.Next(5, 150)), 2),
                { Nombre: "Servicios y utilidades" } => Math.Round((decimal)(25 + random.Next(10, 100)), 2),
                { Nombre: "Sueldos y comisiones" } => Math.Round((decimal)(1000 + random.Next(0, 800)), 2),
                { Nombre: "Alquiler" } => Math.Round((decimal)(2000 + random.Next(-50, 50)), 2),
                { Nombre: "Marketing y publicidad" } => Math.Round((decimal)(30 + random.Next(20, 200)), 2),
                { Nombre: "Equipos y mantenimiento" } => Math.Round((decimal)(40 + random.Next(20, 150)), 2),
        ...
        foreach (var egreso in egresos) db.Egresos.Add(egreso);
        await db.SaveChangesAsync();
    }
}
